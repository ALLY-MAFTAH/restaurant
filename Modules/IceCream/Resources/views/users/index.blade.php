    ...
    <div class="card">
        ...
        <div class="card-body">
            <div id="collapseTwo" style="width: 100%;border-width:0px" class="accordion-collapse collapse"
                ...
                <div class="accordion-body">
                    <div class="card mb-1 p-2" style="background: var(--form-bg-color)">
                        <form method="POST" action="{{ route('icecreams.add') }}">
                            @csrf
                            <div class="row">
            ...
            <table id="data-tebo1"
                ...
                <tbody>
                    @foreach ($icecreams as $index => $user)
                        <tr>
                            <td>{{ ++$index }}</td>
                            <td>{{ $user->name }}</td>
                            <td>{{ $user->email }}</td>
                            ...
                            <td class="text-center">
                                <form id="toggle-status-form-{{ $user->id }}" method="POST"
                                    action="{{ route('icecreams.toggle-status', $user) }}">
                            ...
                            <td>
                                <a href="{{ route('icecreams.show', $user) }}" class="btn btn-sm btn-outline-info collapsed"
                                    type="button">
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                <form method="POST" action="{{ route('icecreams.edit', $user) }}">
                                                    @method('PUT')
                                                    @csrf
                            ...
                            <td class="text-center">
                                ...
                                <form id="delete-user-{{ $user->id }}" method="post"
                                    action="{{ route('icecreams.delete', $user) }}">@csrf @method('delete')
                                </form>
                            </td>
                        </tr>
                    @endforeach
